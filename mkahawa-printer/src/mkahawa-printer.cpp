...
#ifdef DEBUG_PRINT
    printf("pollPrinting(): Log file has disappeared\n");
#endif
    return;
  }
...
#ifdef DEBUG_PRINT
  printf("Initial Size: [%d]  After size[%d]\n", fst.st_size, tstat.st_size);
#endif
...
#ifdef DEBUG
    printf("InitPrintPolling: Started Polling: [%d]\n", n);
#endif
  }
  else{
#ifdef DEBUG
...
#ifdef DEBUG
  printf("doUpdate(): Update Completed: %s [mode=%X]\n", fname.text(), 
	 fmode);
#endif
